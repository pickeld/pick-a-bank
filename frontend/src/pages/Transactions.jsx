  ...
  return (
    <div className="p-4 md:p-6 space-y-4 md:space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-white">פעולות 📋</h1>
          <p className="text-xs md:text-sm text-gray-500 mt-0.5">{total.toLocaleString()} פעולות סה"כ</p>
        </div>
        <div className="flex items-center gap-2">
          {/* Mobile: filter toggle */}
          <button onClick={() => setShowFilters(s => !s)}
            className="md:hidden flex items-center gap-1.5 px-3 py-2 bg-gray-800 hover:bg-gray-700 text-sm text-gray-300 rounded-lg border border-gray-700 transition-colors">
            {showFilters ? <X size={15} /> : <SlidersHorizontal size={15} />}
          </button>
          <button onClick={exportCsv}
            className="flex items-center gap-1.5 px-3 py-2 bg-gray-800 hover:bg-gray-700 text-sm text-gray-300 hover:text-white rounded-lg border border-gray-700 transition-colors">
            <Download size={15} />
      ...
      {/* Billing cycle pills — always visible on mobile */}
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-xs text-gray-500">מחזור:</span>
      ...
      {/* Filters panel — always on desktop, toggle on mobile */}
      <div className={`bg-gray-800 rounded-xl p-4 space-y-3 ${showFilters ? 'block' : 'hidden md:block'}`}>
        {/* Search */}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <input type="date" name="from" value={filters.from}
            ...
            className="flex-1 min-w-0 bg-gray-900 border border-gray-700 rounded-lg px-3 py-1.5 text-sm text-white focus:outline-none focus:border-blue-500" />
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          <button onClick={() => load(page - 1)} disabled={page === 1}
            className="px-3 py-2 text-sm bg-gray-800 hover:bg-gray-700 disabled:opacity-40 text-gray-300 rounded-lg transition-colors">
            ← הקודם
          </button>
