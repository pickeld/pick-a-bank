...
import { Save, Eye, EyeOff, CheckCircle, AlertCircle, Bell, BellOff, Key, CreditCard } from 'lucide-react'
...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    // Fetch distinct cards from transactions
  ...
  const enablePush = async () => {
    try {
      ...
      showToast(true, 'התראות הופעלו ✓')
    } catch (e) { showToast(false, 'שגיאה בהפעלת התראות: ' + e.message) }
  }
    ...
    <div className="p-6 max-w-2xl space-y-6">
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        ...
        <Section title="מפתח OpenAI" badge="AI" badgeColor="green">
          <p className="text-xs text-gray-500 mb-2">מפתח API אישי לתכונות AI (תובנות, המלצות). לא משותף עם אחרים.</p>
          <div className="relative">
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">קבל התראה כשנקלטות עסקאות חדשות בסריקה</p>
              </div>
            </label>
            <label className="flex items-center gap-3 cursor-pointer">
              <input type="checkbox" name="notify_daily_digest" checked={!!form.notify_daily_digest} onChange={onChange}
                className="w-4 h-4 accent-blue-500" />
              <div>
                <p className="text-sm text-white">דוח יומי בשעה 08:00</p>
            ...
            <button type="button" onClick={enablePush}
              ...
              {pushGranted ? <><Bell size={15} /> התראות פעילות</> : <><BellOff size={15} /> הפעל התראות</>}
            </button>
          </div>
        </Section>
        ...
        <div className="bg-gray-800 rounded-xl p-6">
          ...
          <div className="flex items-center gap-4">
            <input type="range" min="1" max="24" step="1"
              name="scrape_interval_hours" value={form.scrape_interval_hours} onChange={onChange}
              className="flex-1 accent-blue-500" />
            <span className="text-lg font-bold text-blue-400 w-12 text-center">{form.scrape_interval_hours}h</span>
          </div>
